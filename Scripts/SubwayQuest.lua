...
local author = "Melt"
local description = [[This script will clear the subway quest]]
...
local SQ  = ScriptBase:new()

function SQ:new()
    return ScriptBase.new(SQ, name, author, description)
end

function SQ:isDoable()
    return Game.assert(hasItem("Bicycle"), "Bot does not have Bicycle") and Game.assert((getMoney() > 50000), "Not enough Money") and (Game.hasPokemonWithMove("Thief") or Game.hasPokemonWithMove("Covet"))
end
...
stepAction[8] = function() log("Subway quest completed.") end
...
function SQ:onPathAction()
    stepAction[qStep]()
end

function SQ:onBattleAction()
    if qStep ~= 5 then
...
function SQ:onDialogMessage(message)
